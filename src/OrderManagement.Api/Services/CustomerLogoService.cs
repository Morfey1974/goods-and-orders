...
public class CustomerLogoService(IWebHostEnvironment env, IConfiguration config)
{
    ...
    public string BuildRelativePath(Guid tenantId, Guid customerId, string extension) =>
        $"{tenantId:N}/customers/{customerId:N}{extension.ToLowerInvariant()}";
        ...
        CancellationToken ct)
    {
        ...
        var tenantDir = Path.Combine(UploadsRoot, tenantId.ToString("N"), "customers");
        Directory.CreateDirectory(tenantDir);

        var relative = BuildRelativePath(tenantId, customerId, ext);
