...
| **Список закупок** | Для накладных в USD — **₪ основная сумма** и **USD вторичной строкой** (по строкам накладной); **сортировка по колонкам** (клик по заголовку ⇅/▲/▼) |
| **Сумма строки при оприходовании** | Достаточно **суммы в USD** или **₪/ед.** (ручной ввод в шекелях, напр. для услуги доставки) |
| **Вложения** | Несколько PDF/изображений на накладную; полоска переключения; предпросмотр в iframe; в списке — 📎 × количество. На **новой** накладной файлы сначала попадают в отложенный список и загружаются на сервер только при сохранении или оприходовании |
...
| **Выбор товаров** | Модальное окно по образцу מחירון: фильтры в шапке, отметка строк, **Сохранить** добавляет позиции в накладную; открытие карточки товара из пикера **не сбрасывает** зелёную отметку строк |
...
| Возможность | Описание |
|-------------|----------|
| **Группы товаров** | Справочник групп на странице «Товары и услуги»; фильтр и привязка товаров к группам (мультивыбор в карточке); **«Добавить группу»** прямо в выпадающем списке групп в карточке товара |
...
| **Колонки** | Перетаскивание границ заголовков; значения по центру (кроме названия/товара/клиента); на **Закупках** — сортировка по клику на заголовок |
...
| Возможность | Описание |
|-------------|----------|
| Склад | Остатки по складам; **артикул + старый מק"ט** в таблице; приход, движения; **та же панель списка**, что и на других вкладках |
