...
| **Оприходование** | Кнопка **«Оприходовать»** сохраняет черновик, проводит товары на склады (движения `Receipt`), закрывает редактор и возвращает в список «Закупки»; перед проведением — подтверждение |
...
| **Карточка товара** | Редактирование **цены продажи**, групп, склада, BOM; вкладки **«Партии FIFO»** и **«Движения склада»** (с себестоимостью ₪); **старый מק"ט** (`legacySku`) под артикулом; выпадающие списки склада/групп **сворачиваются после выбора**; **«Сохранить»** не закрывает окно |
...
На вкладках **Заказы**, **Клиенты**, **Поставщики**, **Товары и услуги**, **Закупки**, **Документы**, **Склад**, **Оценка склада**, **Отчёт по доходам** и **Отчёт по расходам** используется общий компонент `DataTablePanel`:
...
| **Отчёты** (вкладка «Отчёты») | PDF с шапкой бизнеса на иврите; вкладка **«Финансы»** — отчёты по **доходам** и **расходам** за период |
...
| **Расходы за период** | Оприходованные приходные накладные (GR) по дате документа; услуги (доставка и т.п.) — отдельными накладными; тот же фильтр дат и **«Отчёт PDF»** |
...
- `GET /api/reports/expenses?from=&to=` — отчёт по расходам (оприходованные GR по дате документа, итог ₪)
...
| GET | `/api/reports/income/pdf?from=&to=` | PDF отчёта по доходам |
| GET | `/api/reports/expenses?from=&to=` | Отчёт по расходам (оприходованные GR) |
| GET | `/api/reports/expenses/pdf?from=&to=` | PDF отчёта по расходам |
