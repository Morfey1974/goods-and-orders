...
| **Типы FA / CM** | **FA** — основное средство: без склада, амортизация (категория פחת и % бизнеса в карточке товара); **CM** — расходник: списание в журнал расходов при оприходовании |
| **Landed cost на ОС** | Доп. расходы распределяются между складскими строками и **FA** пропорционально сумме |
...
| **Карточка товара** | Редактирование **цены продажи**, групп, склада, BOM; для FA — блок амортизации; вкладки **«Партии FIFO»** и **«Движения склада»** (с себестоимостью ₪); **старый מק"ט** (`legacySku`) под артикулом; выпадающие списки склада/групп **сворачиваются после выбора**; **«Сохранить»** не закрывает окно |
...
| **Операционные** | Бухгалтер, аренда, реклама, **материалы (CM из накладных)** и др. |
...
| **Отчёты** (вкладка «Отчёты») | PDF с шапкой бизнеса на иврите; вкладка **«Финансы»** — доходы, закупки на склад, COGS, валовая прибыль, операционные расходы, **טופס 1342 (יא) — амортизация** |
...
| **Расходы за период** | Оприходованные **складские** закупки (GR) по дате документа — **без FA/CM**; услуги — отдельными накладными; тот же фильтр дат и **«Отчёт PDF»** |
| **COGS / валовая прибыль** | Себестоимость проданного и валовая прибыль за период |
| **Операционные расходы** | Журнал расходов + амортизация ОС за период |
