...
| **Курс USD/ILS** | Автоподстановка курса Банка Израиля на дату документа (`GET /api/exchange-rates/usd-ils`); опционально **«Свой курс USD»** — сохраняется в черновике и используется при пересчёте ₪ и оприходовании |
...
| **Карточка товара** | Редактирование **цены продажи**, групп, склада; для **FG/BD** — **состав (BOM)**: модальное окно выбора CP/SP (как в накладных), таблица артикул / наименование / количество; для FA — блок амортизации; вкладки **«Партии FIFO»** и **«Движения склада»**; **«Сохранить»** не закрывает окно |
...
На вкладках **Заказы**, **Клиенты**, **Поставщики**, **Товары и услуги**, **Закупки**, **Документы**, **Склад**, **Журнал расходов**, **Оценка склада** и финансовые отчёты используется общий компонент `DataTablePanel`:
...
| **Симметричное расширение** | Панель центрируется; угол следует за курсором **1:1** при перетаскивании |
...
| **Оформление** | Скруглённые поля фильтров; шапка и строки таблицы — отдельные блоки с зазором |
...
| **PDF** | Журнал за период — скачивание / предпросмотр |
...
| **Оценка склада** | Отчёты → стоимость остатков; опция **«Детализация по партиям FIFO»**; **скачивание PDF** отчёта; таблица в `DataTablePanel` |
...
| **Доходы за период** | Проведённые קבלות, строки оплаты по дате (кассовый метод); фильтр дат как в YeshInvoice (быстрые кнопки + выпадающий список); кнопка **«Отчёт PDF»** — просмотр в модалке (печать / сохранение / прокрутка) |
| **Расходы за период** | Оприходованные **складские** закупки (GR) по дате документа — **без FA/CM/SV**; строки **SV/CH** (доставка, подписки) — в журнал операционных расходов; тот же фильтр дат и **«Отчёт PDF»** |
| **COGS / валовая прибыль** | Себестоимость проданного и валовая прибыль за период |
| **Операционные расходы** | Журнал расходов + амортизация ОС за период; кнопка **«Отчёт PDF»** |
| **Прибыль и убыток** | Доход − COGS − признанные операционные расходы (журнал + פחת) |
| **Услуги поставщиков** | Landed cost на накладных и строки SV/CH из GR за период |
| **טופס 1342** | Ведомость основных средств за **год מס**: прямая линия, пропорция по месяцам, в последний период — остаток в ноль; экспорт PDF |
...
- `GET /api/reports/profit-and-loss?from=&to=` — прибыль/убыток (עוסק פטור)
...
| Тип | PDF / UI |
|-----|----------|
| הצעת מחיר (Quote) | PDF с баннером «לפרויקט …»; статус **Открыто** до отправки по email; таблица позиций товара |
...
- **«Сохранить черновик»** — без закрытия; **«Сохранить и выйти»** — сохранить и закрыть; **Отмена** / **×** — с подтверждением. Enter в полях даты **не** закрывает окно.
...
| GET | `/api/reports/profit-and-loss?from=&to=` | Прибыль/убыток |
